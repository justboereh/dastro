...
watch(drawerEnabled, (value) => {
    const intent = value ? 'add' : 'remove'
    document.documentElement.classList[intent]('drawer-enabled')
})
</script>

<template>
    <button
        class="w-10 h-10 grid place-items-center border rounded-lg transition duration-200 sm:hidden"
        :class="
            drawerEnabled
                ? 'border-dim-300 dark:border-dark-50'
                : 'border-transparent'
        "
